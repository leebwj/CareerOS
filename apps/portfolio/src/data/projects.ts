...
export const projects: Project[] = [
  ...
  // ── work experience — concise, NDA-safe role pages (à la emmiwu.com/t-mobile) ──
  {
    ...
    metrics: [
      ...
      { value: "91%", label: "notification delivery reach" },
      { value: "~2.7×", label: "more reachable families" },
    ],
    blocks: [
      ...
      { type: "list", label: "What I worked on", items: [
        "Delivered an app-wide React Native redesign from Figma to production — information architecture sequenced ahead of the visual system so users kept their bearings, then a 33-section design system carried onto every screen",
        "Built the product's lifecycle notification system end to end: re-engagement, streak reminders and class-completion alerts, with per-stage copy, back-off between types, and a 20% holdout so the effect could be measured",
        "Made it autonomous — it proposes sends, clears its own safety guardrails and delivers unattended, reaching 91% of targeted devices and producing the first paid conversion the re-engagement flow generated",
        "Widened notification eligibility to every account role after validating it against production data, reaching about 2.7× more families",
        "Closed a notification-injection hole: four public endpoints trusted whatever user ID they were handed, so I bearer-authed them with zero changes for server-to-server callers",
        "Packaged the in-game AI character as a versioned SDK outside studios can build on — typed APIs, per-session isolation and capability enforcement — proven by an external builder shipping a game mode with no access to our repository",
        "Took hosted voice end to end, so a builder can hold a spoken conversation with the character from a browser with no game client",
        "Replaced the blank white crash screen with a branded recovery in English and Korean, and split error from empty on dashboards that had been reporting a failed request as no activity",
      ] },
      { type: "prose", label: "Note", body: [
        "The redesign launched staged at 25% on my last day, so there are no engagement numbers to point at yet. Specifics about the product's internals and metrics stay with the company — happy to talk through the engineering in more depth over a call.",
      ] },
    ],
  },
  {
