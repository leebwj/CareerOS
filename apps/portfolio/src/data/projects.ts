  ...
  // ── work experience — concise, NDA-safe role pages (à la emmiwu.com/t-mobile) ──
  {
    ...
    blocks: [
      ...
      { type: "prose", label: "What I worked on", body: [
        ...
        "Two things nearly went wrong and are the part I would actually defend in a review. The release was planned as an over-the-air update; checking the computed runtime fingerprint against the live binaries showed a dependency upgrade had already invalidated it, so the deploy would have succeeded, reported success, and reached zero users — the team would have spent days believing a feature was live. It was rerouted to a store build. Separately, the fix for a visual defect on the device picker sat outside the feature flag, so the obvious one-line change would have altered the app for the entire user base; putting the geometry behind the flag instead preserved the guarantee the whole rollout rests on, that anyone not in the experiment sees byte-identical output.",
        "The most useful thing I found was that a whole population could never have been tested at all. The rollout flag was scoped by user email, which cannot resolve before sign-in — so signed-out and first-time users were structurally incapable of receiving the new experience, including the app's own login and first-class screens, the exact surfaces new users meet first. Re-scoping it made them reachable.",
        "Ahead of all of it sat a process question the design side had open: how does a design actually become the app? The answer I built was a pipeline that reads a Figma frame and translates it into our own React Native components and theme tokens rather than generic markup, shipped as a skill any engineer can run, with a readiness gate in front of it — every value bound to a variable, layers named semantically, screens handed over one at a time. Two findings came out of using it. The brand kit had quietly drifted from the code tokens, near-miss greys and purples of the kind you get from colour-picking a screenshot. And a screen everyone was treating as a repaint turned out to need data that did not exist yet, which is the general lesson: a redesign is not automatically a reskin, and asking where each value comes from turns a mid-build surprise into a ticket scoped up front.",
        ...
        "I also built the product's lifecycle notification system: the win-back that re-invites lapsed families, streak reminders, and class-completion alerts for parents — with per-stage copy, back-off so the types cannot pile onto the same family, and a 20% holdout so the effect could actually be measured rather than assumed. Version one shipped with a person approving every send, which meant it went dark whenever nobody did — for a month at one point, with no alerting. So I took the person out: it now proposes, checks itself against safety guardrails, and sends unattended, reaching 91% of the devices it targets and producing the first paid conversion the re-engagement flow had ever generated. Eligibility also turned out to be restricted by account role; widening it to every role, validated against production data, reached about 2.7× more families.",
      ...
      { type: "prose", label: "Note", body: [
        "The redesign launched staged at 25% on my last day, so there are no engagement numbers to point at and I am not going to invent any. What I was accountable for is that it reached production safely and can be rolled back instantly, and that the person inheriting the rollout can run it — the ladder, its preconditions, what to watch and what a rollback does not undo all live in the flag's own configuration rather than in my head.",
