...
vec2 stepDir = vec2(dx, dy) / max(maxDelta, 0.001);`,
...

  {
    ...
    blocks: [
      ...
      { type: "code", label: "One point per edge, not per half-edge", file: "mesh.cpp", code: "std::pair<Vertex*, Vertex*> Mesh::getEdgeKey(Vertex* v1, Vertex* v2) const {\n    // order by pointer address so the same undirected edge always\n    // produces the same key, whichever half-edge asked for it\n    if (v1 < v2) return { v1, v2 };\n    return { v2, v1 };\n}", note: "Each edge is two half-edges, so the edge-point pass would compute every point twice and place two vertices where there should be one. Sorting the endpoint pair by pointer address gives both halves the same hash key, so the second one finds the first already there." },
