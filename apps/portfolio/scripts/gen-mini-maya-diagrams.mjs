...
//
// Both were checked against the algorithm before drawing:
...
//    Measured on a real subdivided cube that is FALSE — original vertices keep
//    valence 3, and a face point's valence equals its original face's side count
//    (5 on a dodecahedron). What is true, and what actually explains the
...
const AL = 140, AR = 420, BR_ = 646, AT = 104, AB = 300, ins = 13;
const svgA = `<svg xmlns="http://www.w3.org/2000/svg" width="820" height="392" viewBox="0 0 820 392">
  <rect width="820" height="392" fill="${BG}"/>
  ...
  <g font-family="${MONO}" font-size="13.5">
    ...
    <text x="${(AL + AR) / 2 - 10}" y="${AT + ins - 12}" fill="${ACC}" text-anchor="middle" opacity=".85">he-&gt;next()</text>
    <text x="${AR + ins + 12}" y="${(AT + AB) / 2 + 5}" fill="${DIM}">he-&gt;sym()</text>
    <text x="${AR + 14}" y="${AT - 20}" fill="${ACC}" font-weight="700">he-&gt;vert()</text>
    <text x="${AL + 26}" y="${(AT + AB) / 2 + 5}" fill="${DIM}">he-&gt;face()</text>
  </g>

  <text x="44" y="352" font-family="${SANS}" font-size="14" fill="${INK}">A half-edge is one direction along one edge, owned by exactly one face.</text>
  <text x="44" y="374" font-family="${SANS}" font-size="12.5" fill="${DIM}">next() continues that face's loop — the faint arrows close it and return to he · sym() is the twin in the neighbouring face · vert() is the head</text>
</svg>`;

// ── B. the four passes ──────────────────────────────────────────────────────
const S = 132, PITCH = 192, X0 = 44, TOP = 82;
...
const cap = (i, n, t, f) =>
  `<text x="${bx(i)}" y="${TOP - 38}" font-family="${MONO}" font-size="11" fill="${DIM}" letter-spacing="1.6">${n}</text>
   ...
   <text x="${bx(i)}" y="${TOP + S + 26}" font-family="${SANS}" font-size="12" fill="${DIM}">${f}</text>`;
const IN = 17;
const inner = (i) => [[bx(i) + IN, TOP + IN], [bx(i) + S - IN, TOP + IN], [bx(i) + S - IN, TOP + S - IN], [bx(i) + IN, TOP + S - IN]];

const svgB = `<svg xmlns="http://www.w3.org/2000/svg" width="820" height="330" viewBox="0 0 820 330">
  <rect width="820" height="330" fill="${BG}"/>

  ${cap(0, "01", "face point", "centroid of the face")}
  ${box(0, FAINT)}${CORN(0).map((p) => dot(p, 4, INK)).join("")}${dot(CTR(0), 6, ACC)}

  ${cap(1, "02", "edge points", "2 endpoints + 2 centroids")}
  ${box(1, FAINT)}${CORN(1).map((p) => dot(p, 3.2, FAINT)).join("")}${dot(CTR(1), 3.6, FAINT)}${EDGE(1).map((p) => dot(p, 5.5, ACC)).join("")}

  ${cap(2, "03", "move the originals", "toward their neighbours")}
  ${box(2, FAINT)}
  ...
  ${cap(3, "04", "quadrangulate", "one quad per original corner")}
  ${box(3, INK)}
  ...
  <text x="44" y="288" font-family="${SANS}" font-size="13" fill="${INK}">After one round every face is a quadrilateral. An edge point always has 4 neighbours; a face point has as many as its face had sides.</text>
  <text x="44" y="310" font-family="${SANS}" font-size="13" fill="${DIM}">The originals keep the valence they started with, so irregular vertices never multiply — which is why the surface converges.</text>
</svg>`;
